...
function hasUsableMachineFirstCommand(commandTemplate: string): boolean {
  return isNonEmptyString(commandTemplate)
    && commandTemplate.trim().startsWith(MACHINE_FIRST_COMMAND);
}

function getServices(envelope: Record<string, unknown>): unknown[] | null {
...
function isUsableServiceRow(row: unknown): boolean {
  if (!isRecord(row)) {
    return false;
  }
...
function classifyHardFailure(execution: SkillExecutionRecord): string | null {
...
export function classifyNetworkDirectoryExecution(
  input: ClassifyNetworkDirectoryExecutionInput
): NetworkDirectoryExecutionClassification {
